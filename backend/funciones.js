...
function nuevoUsuario(usuario){
  ...
  .then(function(){
    return users.insertOne(usuario);
  })
}

function comprobarUsuario(nombre, email){
  return users.findOne({'nombre': nombre})
	.then(function(res) {
		if(res){
...
function login(datosLogin){
  return users.findOne({'nombre': datosLogin[0], 'pass': datosLogin[1]})
  .then(function(res){
    if(res){
...
function getUserNameBySessId(sessID){
  ...
  .then(function(res){
    return users.findOne({_id: res.idUser});
  })
  .then(function(res){
    return res.nombre;
  })
